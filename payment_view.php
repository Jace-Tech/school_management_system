						...
						<div class="row">
							...
							<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
								<ul class="breadcome-menu">
									...
									<li><span class="bread-blod">Payment Status</span>
									</li>
								</ul>
							</div>
						</div>
				...
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					<div class="product-status-wrap">
						...
						<div class="asset-inner">
							<?php
							$sql_main = "SELECT * FROM `payment_status`";
							...
							if (isset($_GET["search"])) {
								$search = $_GET['search'];
								$query = "SELECT * FROM `payment_status` WHERE `payment_id` REGEXP ?";
								$result = $conn->prepare($query);
							...
							if (isset($_GET["filter"])) {
								$filter = $_GET['filter'];
								$query = "SELECT * FROM `payment_status` WHERE `session` REGEXP ?";
								$result = $conn->prepare($query);
										...
										<tr>
											...
											<td>
                                                <div style="display: flex; align-items: center;">
                                                    <span style="text-transform: capitalize; "><?php echo $status; ?></span>
                                                    <span style="background-color:<?php if($status !== "paid") echo 'red' ; else echo 'green'; ?>; height: 15px; width: 15px; border-radius: 50%; display: inline-block; margin-left: 10px;"></span>
                                                </div>
                                            </td>

											<td><?php echo $paymentSession; ?></td>

											<td>
												<a href="./handlers/payment.php?a_id=<?= $payment_id; ?>" class="btn btn-sm btn-primary">
													Mark as paid
												</a>
												<a href="./handlers/payment.php?d_id=<?= $payment_id; ?>" style="margin-left: 4px;" class="btn btn-sm btn-danger">
                                                    Mark as unpaid
												</a>
											</td>
										</tr>
